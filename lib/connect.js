    ...
    sock.ev.on("connection.update", async (update) => {
      try {
        ...
        if (connection === "open") {
          ...
          await db.query("UPDATE users SET is_linked = $1", [trueBool]);
          await db.query("UPDATE users SET status = $1", ["active"]);
          const now = new Date();
          const lastConnected = now.toLocaleString()
          await db.query("UPDATE users SET last_connected = $1", [lastConnected]);
        ...
        if (connection === "close") {
          ...
          if (reason === DisconnectReason.loggedOut) {
            ...
            await db.query("UPDATE users SET is_linked = $1", [falseBool]);
