...
const systemInstructions = {
	...
	You're not just a Sophia Assistant. You're *their* Sophia.`,
...
class Message {
    constructor(sock, msg) {
        ...
        this.sock = sock;
        this.system = systemInstructions[f.getTone()];
        this.participant = msg.key.participant
        this.contextInfo = msg.message?.extendedTextMessage?.contextInfo;
        this.quoted = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage
        this.convo = msg.message?.conversation || msg.message?.extendedTextMessage?.text;
    }
...
async getMetaData(jid){
	...
	isGroupCEO() {
	  ...
	  if (this.isGroup()) {
	    return ceoLids.includes(this.participant);
	  } else if(this.isPrivate()){
	  	return ceoJids.includes(this.jid);
	  }
	...
	isQuotedBot(){
		return this.contextInfo.participant === this.sock.user.id.split(":")[0]+"@s.whatsapp.net";
	}
    ...
    async getAIReply() {
      try {
        ...
        const response = await axios.post("http://localhost:5000/ai/reply", {
          chatID: this.jid || "Chat_id_1",
          message: this.convo || "The user sent a media message. Tell him you are unable to see it.",
          system: this.system,
        }, {
          headers: {
        ...
        // Check if tone change is requested
        if (functionCalls && functionCalls[0].name === "change-tone" && functionCalls.length > 0) {
		const functionCall = functionCalls[0];
          const newTone = functionCall.args.tone;
          f.changeTone(newTone)
          const message = `Let the user know the tone change was successful in a ${f.getTone()} way.`;
          
          // Call again after tone change
          const newResponse = await axios.post("http://localhost:5000/ai/reply", {
            chatID: this.jid,
            message: message,
            system: `
            The function call 'change-tone' was completed successfully. The user's preferred tone is now set to '${f.getTone()}'.
            
            From now on, respond in a way that reflects this tone. Keep this in mind while generating all future responses.
    		...
    		console.log("Second response data:", newResponse.data);
          return newResponse.data?.reply || "Done";
        }
