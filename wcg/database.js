...
class WCGDatabase {
  ...
  initTables() {
    // Main games table - UPDATED with current_turn field
    db.prepare(`
  ...
  createGame(groupId) {
    ...
    return db.prepare(`
      INSERT INTO wcg_games (group_id, is_joining, join_ends_at, current_turn)
      VALUES (?, 1, ?, 0)
  ...
  startGame(gameId, startingWord) {
    ...
    db.prepare(`
      ...
      SET is_joining = 0, is_active = 1, current_word = ?, 
          next_letter = ?, used_words = ?, successful_words = 1, current_turn = 0
      WHERE id = ?
  ...
  setCurrentTurn(gameId, turnIndex) {
    db.prepare(`
      UPDATE wcg_games 
      SET current_turn = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
  ...
  updateGameWord(gameId, word, nextLetter) {
    const game = db.prepare(`SELECT used_words, successful_words FROM wcg_games WHERE id = ?`).get(gameId);
    ...
    // Calculate new timer (decrease every 2-4 successful words)
    let newTimer = db.prepare(`SELECT timer_seconds FROM wcg_games WHERE id = ?`).get(gameId).timer_seconds;
    if (successfulWords > 0 && successfulWords % (Math.floor(Math.random() * 3) + 2) === 0) {
      newTimer = Math.max(15, newTimer - 5); // Minimum 15 seconds
    }
