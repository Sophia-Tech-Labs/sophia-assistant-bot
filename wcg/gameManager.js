...
class WCGGameManager {
    ...
    try {
      ...
      if (existingGame) {
        if (existingGame.is_joining) {
          ...
          return {
            success: false,
            message: `Word Chain Game is already active in this group! Current word: "${existingGame.current_word}" | Next letter: "${existingGame.next_letter.toUpperCase()}" | Word size: ${existingGame.word_size}+`
          };
        }
      }
  ...
  async joinGame(groupId, playerJid, playerName, messageInstance) {
    try {
      ...
      if (!game.is_joining) {
        return { success: false, message: "Join period has ended! Game is already in progress." };
      }

      const joined = this.db.addPlayer(game.id, playerJid, playerName || "Player");
  ...
  async finishJoinPeriod(gameId, groupId, messageInstance) {
    try {
      ...
      if (playerCount < 2) {
        ...
        await messageInstance.reply("❌ Not enough players joined! Need at least 2 players to start. Game cancelled.");
        return;
      }
      ...
      const currentPlayerPhone = this.extractPhoneNumber(currentPlayer.player_jid);
      const nextPlayerPhone = this.extractPhoneNumber(nextPlayer.player_jid);
      
      const playerList = players.map((p, index) => `${index + 1}. ${p.player_name}`).join('\n');
      
      const message = `🚀 **GAME STARTED!**\n\n👥 **Players (${players.length}):**\n${playerList}\n\n🎯 **Starting word:** "${startingWord}"\n📝 **Next word must start with:** "${game.next_letter.toUpperCase()}"\n📏 **Minimum word length:** ${game.word_size} letters\n⏰ **Time limit:** ${game.timer_seconds} seconds per turn\n\n🎯 **Current turn:** @${currentPlayerPhone}\n⏭️ **Next turn:** @${nextPlayerPhone}\n\n**Your turn! Give me a word that starts with "${game.next_letter.toUpperCase()}" (${game.word_size}+ letters)**`;
      
      await messageInstance.reply(message, {
        mentions: [`${currentPlayerPhone}@s.whatsapp.net`, `${nextPlayerPhone}@s.whatsapp.net`]
      });
  ...
  async handleTurnTimeout(groupId, messageInstance) {
    try {
      ...
      if (remainingPlayers.length < 2) {
        await messageInstance.reply(`⏰ ${currentPlayer.player_name} timed out and was removed!\n\n❌ Not enough players left! Game ended.`);
      ...
      const afterNextPlayer = remainingPlayers[(newTurnIndex + 1) % remainingPlayers.length];
      
      // Extract phone numbers for mentions
      const currentPlayerPhone = this.extractPhoneNumber(nextPlayer.player_jid);
      const nextPlayerPhone = this.extractPhoneNumber(afterNextPlayer.player_jid);
      
      const message = `⏰ ${currentPlayer.player_name} timed out and was removed! (${remainingPlayers.length} players left)\n\n🎯 **Current turn:** @${currentPlayerPhone}\n⏭️ **Next turn:** @${nextPlayerPhone}\n\n**Your turn! Give me a word that starts with "${game.next_letter.toUpperCase()}" (${game.word_size}+ letters)**`;
  ...
  async skipToNextTurn(groupId, messageInstance, reason = "") {
    try {
      ...
      message += `**Your turn! Give me a word that starts with "${game.next_letter.toUpperCase()}" (${game.word_size}+ letters)**`;
  ...
  async processWord(groupId, playerJid, playerName, word, messageInstance) {
    try {
      ...
      if (this.db.isWordUsed(game.id, validation.word)) {
        await messageInstance.reply(`❌ "${validation.word}" has already been used! Try a different word.`);
        // Give them another chance, restart turn timer
        this.startTurnTimer(groupId, game.timer_seconds, messageInstance);
        return { processed: true };
      }
      ...
      const currentPlayerNewIndex = updatedPlayers.findIndex(p => p.player_jid === playerJid);
      ...
      const updatedPlayer = updatedPlayers.find(p => p.player_jid === playerJid);
      
      const currentPlayer = updatedPlayers[nextTurnIndex];
      const nextPlayer = updatedPlayers[(nextTurnIndex + 1) % updatedPlayers.length];
      ...
      message += `\n\n🎯 **Current turn:** @${currentPlayerPhone}`;
      message += `\n⏭️ **Next turn:** @${nextPlayerPhone}`;
      message += `\n\n**Your turn! Give me a word that starts with "${updatedGame.next_letter.toUpperCase()}" (${updatedGame.word_size}+ letters)**`;
  ...
  async endGame(groupId, messageInstance, endedByUser = false) {
    try {
      ...
      if (winner && winner.score > 0) {
        message += `🏆 **Winner: ${winner.player_name}** (${winner.score} words)\n\n`;
      }
